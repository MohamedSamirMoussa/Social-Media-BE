...
class UserServices {
  ...
  confirmEmail = async (req: Request, res: Response) => {
    ...
    if (!user) {
      throw new BadRequestError("Email isn't exists");
    }
  ...
  signin = async (req: Request, res: Response) => {
    ...
    if (!user) {
      throw new BadRequestError("This account doesn't exists");
    }
    ...
    return successHandler({
      res,
      status: 201,
      message: `User login successfully... Hello, dear ${
  ...
  verifyOTP = async (req: Request, res: Response) => {
    ...
    res.cookie("reset_password_token", resetToken, {
      ...baseCookieOptions,
      maxAge: Number(process.env.RESET_PASSWORD_TOKEN) * 1000,
    });
  ...
  resetPassword = async (req: Request, res: Response) => {
    ...
    if (!user.confirmedAt) {
      throw new ConflictError("Email not confirmed");
    }
    ...
    res.clearCookie("reset_password_token", clearCookieOptions);
  ...
  getMe = async (req: Request, res: Response) => {
    if (!req.user) {
      throw new NotAuthorizedError("User isn't logged");
    }
  ...
  logout = async (req: Request, res: Response) => {
    ...
    res.clearCookie("signature_level", clearCookieOptions);
